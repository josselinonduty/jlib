...
echo ""
echo "<sub>Last modification: $(git log -1 --pretty='format:%cd' --no-merges --date=format:'%Y-%m-%d %H:%M' ${RELEASE_TAG})</sub>"

echo ""
echo "## Commits"
echo ""

git log --no-merges --first-parent --pretty=format:"- %s (%an <%ae>)" "${PREVIOUS_TAG}..${RELEASE_TAG}"
